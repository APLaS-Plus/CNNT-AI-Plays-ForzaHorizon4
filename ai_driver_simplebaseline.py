...
class AIDriverSimpleBaseline:
    """
    AI driver for Forza Horizon 4 using SimpleBaseline model for control prediction
    ...
    def __init__(self):
        ...
        # Initialize SimpleBaseline model with 224x128 input size
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.model = SimpleBaselineModel(
            img_size=(224, 128),  # 使用224x128分辨率
            patch_size=4,
            window_size=7,  # 使用与训练时相同的窗口大小
            embed_dim=96,
            depths=[2, 2, 6, 2],
            num_heads=[3, 6, 12, 24]
        )
        self.load_simplebaseline_model()
    ...
    def load_simplebaseline_model(self):
        """Load the SimpleBaseline model"""
        model_path = (
            ROOT_DIR / "model" / "SimpleBaseline" / "best_baseline_model.pth"
        )
        try:
            self.model.load_state_dict(
                torch.load(str(model_path), map_location=self.device)
            )
            print("Successfully loaded SimpleBaseline model weights")
        except Exception as e:
            print(f"Error loading SimpleBaseline model: {e}")
            print("Using randomly initialized SimpleBaseline model")
    ...
    def process_frame(self):
        ...
        with self.timer.measure("总处理时间"):
            ...
            # Convert guideline image for model input - 添加尺寸转换
    ...
    def drive_loop(self):
        ...
        frame_count = 0
        warmup_frames = 5  # Reduced warmup frames for SimpleBaseline
    ...
    def start(self):
        ...
        print("Starting SimpleBaseline AI driver...")
        self.running = True
...
def main():
    ...
    print("Initializing SimpleBaseline AI driver for Forza Horizon 4...")
    driver = AIDriverSimpleBaseline()
