...
if __name__ == "__main__":
    ...
    easyocr_reader = easyocr.Reader(gpu=True, lang_list=['en'], model_storage_directory=str(MODEL_PATH))
    ...
    for i in range(len(image_files)):
        ...
        digit = easyocr_reader.recognize(digit_region, batch_size=10)[0][1]
        digit = list(digit)
        for j in range(len(digit)):
            if digit[j] == "g":
                ...
                digit[j] = '1'
            elif digit[j] == "z" or digit[j] == "Z" or digit[j] == "r" or digit[j] == "]":
                digit[j] = '2'
                ...
                digit[j] = '8'
            elif digit[j] == "q" or digit[j] == "Q" or digit[j] == "g" or digit[j] == "j":
                digit[j] = '9'
        ...
        for j in range(len(digit)):
            if digit[j] not in "0123456789":
                print(digit)
                print(f"Warning: {digit} is not a valid digit. Please check the image: {img_path}")
    ...
    for image in train_images:
        shutil.move(os.path.join(DATASET_DIR, image), os.path.join(DATASET_DIR, "train", image))
        shutil.move(os.path.join(DATASET_DIR, image.replace('.jpg', '.txt')), os.path.join(DATASET_DIR, "train", image.replace('.jpg', '.txt')))
    for image in val_images:
        shutil.move(os.path.join(DATASET_DIR, image), os.path.join(DATASET_DIR, "val", image))
        shutil.move(os.path.join(DATASET_DIR, image.replace('.jpg', '.txt')), os.path.join(DATASET_DIR, "val", image.replace('.jpg', '.txt')))
