...
class SimpleBaselineModel(nn.Module):
    """Swin Transformer based model for driving prediction"""

    def __init__(
        self,
        img_size=(224, 128),  # 使用224x128作为默认分辨率
        patch_size=4,
        in_chans=1,
        embed_dim=96,
        depths=[2, 2, 6, 2],
        ...
        print(f"Patch resolution: {patches_resolution}, Window size: {window_size}")
        # 验证窗口大小是否合适 - 224÷4=56, 128÷4=32
        # Layer 0: 56x32, Layer 1: 28x16, Layer 2: 14x8, Layer 3: 7x4
        for i in range(self.num_layers):
            h_res = patches_resolution[0] // (2**i)
            w_res = patches_resolution[1] // (2**i)
            print(f"Layer {i}: resolution {h_res}x{w_res}")
            if h_res < window_size or w_res < window_size:
                print(
                    f"Warning: Layer {i} resolution {h_res}x{w_res} is smaller than window size {window_size}"
                )
                print(
                    f"Window size will be automatically adjusted to {min(h_res, w_res)}"
                )
        ...
        self.regressor = nn.Sequential(
            nn.Linear(final_dim + 1, 512),  # +1 for speed
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(512, 256),
            nn.ReLU(),
